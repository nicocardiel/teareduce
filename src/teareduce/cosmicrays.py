    ...
    if debug_level > 0:
        if zoom_region_imshow is None:
            ...
            if isinstance(zoom_region_imshow, SliceRegion2D):
                ...
                raise ValueError(
                    f"Object zoom_region_imshow={zoom_region_imshow} "
        ...
        image_list1 = [data1, labels_pos_peak, labels_pos_tail, labels_pos_tail_in_peak, data1, data1c]
        title_list1 = [
            "data1",
            "labels_pos_peak",
            "labels_pos_tail",
            "labels_pos_tail_in_peak",
            "data1 with C.R.",
            "data1c",
        ]
        if debug_level == 1:
        ...
        if single_mode:
            ...
            image_list2 = [data2, labels_neg_peak, labels_neg_tail, labels_neg_tail_in_peak, data2, data2c]
            title_list2 = [
                "data2",
                "labels_neg_peak",
                "labels_neg_tail",
                "labels_neg_tail_in_peak",
                "data2 with C.R.",
                "data2c",
            ]
            if debug_level == 1:
                    ...
                    if list_skipped_regions is not None:
                        for region2d in list_skipped_regions:
                            ...
                            if iblock == 1:
                                x1 = x1 - ioffx
                                y1 = y1 - ioffy
                            rect = patches.Rectangle((x1, y1), xwidth, yheight, edgecolor="yellow", facecolor="none")
                            ax.add_patch(rect)
                    elif image_region is not None:
                        ...
                        if iblock == 1:
                            x1 = x1 - ioffx
                            y1 = y1 - ioffy
                        rect = patches.Rectangle((x1, y1), xwidth, yheight, edgecolor="cyan", facecolor="none")
                        ax.add_patch(rect)
                plt.tight_layout()
                plt.show()
...
def apply_cr2images_ccddata(
    ...
    if infile2 is not None:
        ...
        ccdimage1_clean.uncertainty.array[ccdimage1_clean.mask] = ccdimage2.uncertainty.array[ccdimage1_clean.mask]
        ccdimage2_clean.uncertainty.array[ccdimage2_clean.mask] = ccdimage1.uncertainty.array[ccdimage2_clean.mask]
        ...
        ccdimage1_clean.uncertainty.array[ccdimage1_clean.mask] = ccdimage2_uncertainty_array[ccdimage1_clean.mask]
        history_list.append(f"- infile1: {Path(infile1).name}")
    ...
    for outfile, ccdimage_clean in zip([outfile1, outfile2], [ccdimage1_clean, ccdimage2_clean]):
...
def crmedian(inputlist):
    ...
    image3d = np.ma.array(
        data=np.zeros((num_images, naxis2, naxis1), dtype=float), mask=np.ones((num_images, naxis2, naxis1), dtype=bool)
    )
