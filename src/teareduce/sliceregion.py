...
class SliceRegion1D:
    ...
    def __repr__(self):
        if self.mode == "fits":
            return f"{self.__class__.__name__}(" f'{self.fits!r}, mode="fits")'
        else:
            return f"{self.__class__.__name__}(" f'{self.python!r}, mode="python")'
...
class SliceRegion3D:
    ...
    def __init__(self, region, mode=None, naxis1=None, naxis2=None, naxis3=None):
        ...
        if isinstance(region, str):
            ...
            numbers_int = list(map(int, numbers_str))
            region = np.s_[
                numbers_int[0] : numbers_int[1], numbers_int[2] : numbers_int[3], numbers_int[4] : numbers_int[5]
            ]
